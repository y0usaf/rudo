...
const TERMINAL_SELECTION_ALPHA: f32 = 0.35;
...
pub struct Renderer {
    ...
    terminal_selection: Option<TerminalSelectionView>,
}
...
impl Renderer {
    ...
    pub fn set_terminal_selection(&mut self, selection: Option<TerminalSelectionView>) {
    ...
    fn terminal_selection_window(&self) -> Option<(&TerminalSelectionView, &RenderedWindow)> {
        let selection = self.terminal_selection.as_ref()?;
        let window = self.rendered_windows.get(&selection.grid_id)?;
        Some((selection, window))
    }

    fn draw_terminal_selection(&mut self, root_canvas: &Canvas, grid_scale: GridScale) {
        let Some((selection, window)) = self.terminal_selection_window() else {
            return;
        };
        ...
        paint.set_alpha_f(TERMINAL_SELECTION_ALPHA);
    ...
    pub fn animate_frame(&mut self, grid_rect: &GridRect<f32>, dt: f32) -> bool {
        ...
        let mut animating =
            windows.fold(false, |acc, window| acc | window.animate(grid_rect, dt));
    ...
    pub fn handle_draw_commands(&mut self, batch: Vec<DrawCommand>) -> DrawCommandResult {
        ...
        for draw_command in batch {
            ...
            tracy_named_frame!("draw batch processed");
        }
        self.flush();
    ...
    pub fn flush(&mut self) {
        self.rendered_windows.iter_mut().for_each(|(_, w)| w.flush());
    }
